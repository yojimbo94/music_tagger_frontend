...
function TrackDetailsModal({ track, onClose, onUpdateDiscogs }) {
    ...
    const handleSearchDiscogs = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok) throw new Error(data.error || 'Erreur lors de la mise à jour Discogs')
            ...
            setSearchResults([
                {
                    ...
                    thumbnail: updatedTrack.discord_image || 'https://via.placeholder.com/150x150?text=Album',
    ...
    const trackInfo = useMemo(() => {
        const baseRows = [
            ...
            { type: 'row', key: 'created-at', label: 'Date de création', value: new Date(track.created_at).toLocaleString('fr-FR') }
        ]

        if (track.status === 'matched') {
            return [
                ...baseRows,
                { type: 'row', key: 'status', label: 'Statut', value: <span className="text-green-600 font-medium">✓ Matché</span> },
        ...
        return [
            ...baseRows,
            { type: 'row', key: 'status', label: 'Statut', value: <span className="text-red-600 font-medium">✗ Échoué</span> }
        ]
    }, [track])

    const title = track.status === 'matched' ? track.discogs_album || track.source_title : track.source_title
    const subtitle = `${track.discogs_artist || 'Artiste inconnu'}${track.year ? ` · ${track.year}` : ''}`
                        ...
                        <div className="space-y-4">
                            ...
                            {mediaPlayer && (
                                <div className="rounded-lg border border-gray-200 bg-white p-3 shadow-sm w-full">
                                    ...
                                    <div className="flex justify-center"> {/* Centrer le lecteur */}
                                        {mediaPlayer.type === 'spotify' ? (
                                            <div className="w-full max-w-[280px]"> {/* Limiter la largeur maximale */}
                                                <iframe
                                                    src={mediaPlayer.src}
                                                    width="100%"
